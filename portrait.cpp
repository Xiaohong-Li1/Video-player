    ...
    ui(new Ui::Portrait)
{
	...
	ui->btnPlay_5->installEventFilter(this);
	ui->btnPlay_4->installEventFilter(this);


    ui->btnAdd->installEventFilter(this);
    ui->btnAdd_File->installEventFilter(this);
    ui->checkBox_DQ->installEventFilter(this);
    ui->checkBox_LB->installEventFilter(this);
...
void Portrait::onstateChanged(QMediaPlayer::State state)
{//播放器状态变化
...
void Portrait::playstate(QMediaPlayer::MediaStatus status)
{
...
void Portrait::onDurationChanged(qint64 duration)
{//文件时长变化
...
void Portrait::onPositionChanged(qint64 position)
{//文件播放位置变化
...
void Portrait::on_pushButton_5_clicked()
{
...
void Portrait::on_pushButton_4_clicked()
{
...
void Portrait::on_pushButton_6_clicked()
{
    ChangeLanguage();
}
...
void Portrait::on_pushButton_7_clicked()
{
    emit ChangeRatio_R();
}


bool Portrait::eventFilter(QObject *obj, QEvent *e)
{
		...
		|| e->type() == QEvent::HoverMove)
	{
		...
		if (obj == ui->btnPlay_4)
		{
			if (m_change)
			{
				QToolTip::showText(QCursor::pos(), "list ratio");
			}
			else
			{
				QToolTip::showText(QCursor::pos(), "列表");
        ...
        if (obj == ui->btnAdd)
        {
            if (m_change)
            {
                QToolTip::showText(QCursor::pos(), "Select Video");
            }
            else
            {
                QToolTip::showText(QCursor::pos(), "选择视频");
            }

        }
        if (obj == ui->btnAdd_File)
        {
            if (m_change)
            {
                QToolTip::showText(QCursor::pos(), "Select File");
            }
            else
            {
                QToolTip::showText(QCursor::pos(), "选择文件");
            }

        }
        if (obj == ui->checkBox_DQ)
        {
            if (m_change)
            {
                QToolTip::showText(QCursor::pos(), "Single Cycle");
            }
            else
            {
                QToolTip::showText(QCursor::pos(), "单集循环");
            }

        }
        if (obj == ui->checkBox_LB)
        {
            if (m_change)
            {
                QToolTip::showText(QCursor::pos(), "List Cycle");
            }
            else
            {
                QToolTip::showText(QCursor::pos(), "列表循环");
            }
...
void Portrait::ChangeLanguage()
{
    ...
    if (m_change)
    {
        ...
        ui->btnAdd->setText("");
        ui->btnAdd_File->setText("");
        ui->checkBox_DQ->setText("");
        ui->checkBox_LB->setText("");
...
void Portrait::on_checkBox_LB_clicked(bool checked)
{
...
void Portrait::on_checkBox_DQ_clicked(bool checked)
{
    ui->checkBox_DQ->setChecked(checked);
    ui->checkBox_LB->setChecked(!checked);
...
void Portrait::on_btnAdd_File_clicked()
{
...
void Portrait::on_btnAdd_clicked()
{//打开文件
...
void Portrait::on_pushButton_2_clicked()
{
...
void Portrait::on_btnPlay_clicked()
{//播放
    player->play();
}

void Portrait::on_btnPause_clicked()
{//暂停
    player->pause();
}
...
void Portrait::on_btnStop_clicked()
{//停止
    player->stop();
}


void Portrait::on_btnPlay_2_clicked()
{
    curentVideoNum--;
    if(curentVideoNum < 0)
    {
...
void Portrait::on_btnPlay_3_clicked()
{
    curentVideoNum++;
    if(curentVideoNum >= VideoNameList.length())
    {
...
void Portrait::on_btnSound_clicked()
{//静音按钮
...
void Portrait::on_sliderPosition_valueChanged(int value)
{//播放位置
    player->setPosition(value);
}


void Portrait::on_sliderVolumn_valueChanged(int value)
{//调节音量
    player->setVolume(value);
}


void Portrait::on_btnPlay_5_clicked()
{
    QRect _r= ui->groupBox_2->geometry();
    if (_r.x() > 860)
    {
        ui->groupBox_2->setGeometry(860 - _r.width(), 130, _r.width(), _r.height());
        ui->groupBox_2->raise();
    }
    else
    {
        ui->groupBox_2->setGeometry(1200, 130, _r.width(), _r.height());
    }

	ui->groupBox->setGeometry(1200, 130, _r.width(), _r.height());
	
}


void Portrait::on_btnPlay_4_clicked()
{
	QRect _r = ui->groupBox->geometry();
	if (_r.x() > 860)
	{
		ui->groupBox->setGeometry(860 - _r.width(), 130, _r.width(), _r.height());
        ui->groupBox->raise();
	}
	else
	{
...
void Portrait::on_radioButton_2_clicked(bool checked)
{
...
void Portrait::on_radioButton_clicked(bool checked)
{
...
void Portrait::on_radioButton_5_clicked(bool checked)
{
...
void Portrait::on_radioButton_6_clicked(bool checked)
{
    ui->videoWidget->setAspectRatioMode(Qt::KeepAspectRatio);
...
void Portrait::on_pushButton_clicked()
{//全屏按钮
...
void Portrait::on_pushButton_3_clicked()
{
...
void Portrait::on_listWidget_doubleClicked(const QModelIndex &index)
{
...
void Portrait::on_comboBox_currentTextChanged(const QString &arg1)
{
